...
function blurCheck(event) {
	...
	if (name == 'username') {
		...
		if (str.match(/^[1-9][0-9]{7}$/) == null) {
			message[1].textContent = '学号8位数字，不能以0开头';
		}		
	} else if (name == 'phone') {
		if (str.match(/^[1-9][0-9]{10}$/) == null) {
			message[2].textContent = '电话11位数字，不能以0开头';
		}
	} else if (name == 'email') {
		if (str.match(/^[a-zA-z0-9_\-]+@(([a-zA-z0-9_\-])+\.)+[a-zA-z]{2,4}$/) == null) {
			message[3].textContent = '邮箱格式不符';
		}
	}
}
...
function focusCheck(event) {
	...
	else if (name == 'studentId') message[1].textContent = '';
	else if (name == 'phone') message[2].textContent = '';
	else if (name == 'email') message[3].textContent = '';
}

function formSubmit() {
	if (message[0].textContent != ''||message[1].textContent != ''||message[2].textContent != ''||message[3].textContent != '')
	return false;
}